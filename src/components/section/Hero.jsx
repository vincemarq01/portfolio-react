...
const Hero = () => {
	return (
		<>
			<div id="hero" className="flex max-w-[1000px] mx-5  md:mx-auto  w-full min-h-screen z-0 overflow-hidden">
				<div className="my-auto text-white space-y-5">
					...
					<motion.div variants={fadeIn("right", 1, 0.8)} initial="hidden" animate="show" viewport={{ once: false, amount: 0.7 }} className="flex flex-row gap-1 ">
						<FaGithub size={50} color="black" className="cursor-pointer  hover:bg-secondary p-3 rounded-md" />

						<FaLinkedin size={50} color="black" className="cursor-pointer  hover:bg-secondary p-3 rounded-md" />
					</motion.div>
				</div>
			</div>
		</>
